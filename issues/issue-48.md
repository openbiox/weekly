...
所有能够度量的性状都可以称为数量性状(QT)，这些性状呈连续的，不可以严格的分类，而数量性状基因座（QTL）即控制数量性状的基因在基因组中的位置，而表达数量性状基因座（eQTL）就是能控制数量性状基因的表达水平高低的基因在基因组中的位置。近日近日，美国布莱根妇女医院与哈佛医学院Soumya Raychaudhuri研究组在Nature上发表了文章，聚焦记忆T细胞，在单细胞分辨率水平建立了的eQTL模型，捕获细胞状态依赖的基因调控变异。
